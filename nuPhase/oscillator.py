...
class OscillationCalculator:
    ...
    def __init__(
        ...
        if initialisation == "zeros":
            ...
            self.parameters["theta12"] = Tensor(
                [33.41 * m.pi / 180.0], requires_grad=True
            )
            self.parameters["theta23"] = Tensor(
                [49.1 * m.pi / 180.0], requires_grad=True
            )
            self.parameters["theta13"] = Tensor(
                [8.54 * m.pi / 180.0], requires_grad=True
            )
            self.parameters["deltacp"] = Tensor(
                [197.0 * m.pi / 180.0], requires_grad=True
            )
            self.parameters["dmsq21"] = Tensor(
                [7.41e-5 * units.eV * units.eV], requires_grad=True
            )
            self.parameters["dmsq32"] = Tensor(
                [2.437e-3 * units.eV * units.eV], requires_grad=True
            )
    ...
    def _setup_propagator(self):
        ...
        self.propagator = (
            DPpropagator(10)
            .set_baseline(self.baseline * units.km)
            .set_antineutrino(False)
            .set_density(self.density)
        )
    ...
    def calculate_osc_probs(
        self, energies: typing.Union[np.ndarray, Tensor], antineutrino: bool = False
    ) -> Tensor:
        ...
        if type(energies) == np.ndarray:
            ...
            raise ValueError(
                "bad type for energies, should be numpy array or nuTens Tensor"
            )
