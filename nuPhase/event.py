...
class Event:
    ...
    def add_particles_from_arrays(
        ...
        for _pdg, _energy, _px, _py, _pz in zip(pdg, energies, px, py, pz):
            ...
            self.particles.append(
                Particle(pdg=_pdg, momentum=(_px, _py, _pz), energy=_energy)
            )
            self.n_particles += 1
    ...
    def get_var(self, var: str):
        ...
        if var == "Enu_true":
            ...
            raise ValueError(
                f"Variable {var} was not found in event with aux variables {self.aux_vars}"
            )
